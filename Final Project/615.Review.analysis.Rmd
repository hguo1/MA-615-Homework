...
$~~~$The study apply text analysis by using text variable and rate variable.  Text is review from users about  business in Yelp.  At first, the study separate the text variable in to words.  In the other words, the study splits sentences into words, and group by the Rate variable.  Since this study wants to analysis review for different Rate.  
```{r, echo = FALSE}
...
$~~~$n is the number of times that word is used in Reviews text and total is the total words in Reviews text. In the followling figure, it shows that  the distribution of n/total for each Rate category.  
