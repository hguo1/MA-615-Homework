...
$~~~~$This study random select 10000 sample from yelp business sample size, and get the numeric information need to be used to do furture analysis.   Next the study remove the missing value.  The study pick the stars of business and review count of business as interested variable and apply cluster analysis to that variable.  The cluster analysis is based on the  stars of business and review count of business.  
...
$~~~~$This data want to cluster data into four clusters, and generate 25 initial configurations.
...
$~~~~$This study also want to plot cluster with the business_id as their label on it.  Based on the plot the study can know that the business from cluster group 1 have zero review count.  Business from cluster group 1 have more review count and higher stars than business cluster group 2.  Business from cluster group 3 have more review count and higher stars than business cluster group 2. Business from cluster group 4 have more review count and higher stars than business cluster group 3.
...
$~~~~$The study are interesting the distribution of business stars with photo or without photo.  The study take a sample data frame from photo jason file in Yelp data set.  This study use leftjoin to combine the photo data frame and business data frame.  Left joint will keep the all row of left data frame.  It means that  if a business_id is not contained in photo data set, The row contain this business id will return NA for photo_id columns. The study trades NA as this business has no photo on Yelp.  
```{r, echo = FALSE}
