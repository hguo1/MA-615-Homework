...
FirstName<-rep(NA,length(dat.normalized$contrib))
for(r in 1:nrow(dat.normalized)){
  FirstName[r]<-strsplit(dat.normalized$contrib[r]," ")[[1]][2]
  
}
dat.normalized$contrib<-paste(sub(" .*", "", dat.normalized$contrib), FirstName, sep=" ")

dat.normalized$contrib[dat.normalized$contrib=="SABAN, HAIM,,"]<-"SABAN, HAIM"

dat.normalized$contrib[dat.normalized$contrib=="SABAN,HAIM NA"]<-"SABAN, HAIM"
...
dat.normalized$contrib[dat.normalized$contrib=="FISH, JOHN,"]<-"FISH, JOHN"
dat.normalized$contrib[dat.normalized$contrib=="FISH,JOHN"]<-"FISH, JOHN"
dat.normalized$contrib[dat.normalized$contrib=="PRITZKER SIMMONS,"]<-"PRITZKER, SIMMONS"
...
Top_Donor<-dbConnect(SQLite(),"Top_Donor")
